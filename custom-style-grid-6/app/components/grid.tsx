...
import { BrainIcon, LinkIcon, TextIcon } from "./icons";
...
const MainSkeleton = () => {
  ...
  return (
    <div className="h-full w-full px-12 flex flex-col gap-4 relative z-20">
      <input
        type="text"
        placeholder="Type your message here..."
        className="w-[calc(100%-4rem)] h-10 bg-white border border-neutral-200 mx-auto absolute bottom-4 inset-x-0 px-4 text-xs rounded-md placeholder-neutral-200"
      />
      {chat.map((message) => {
    ...
    <div className="max-w-5xl py-20 mx-auto border-x border-neutral-200 bg-gray-100 min-h-screen">
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-x divide-y divide-neutral-200">
        <Card>
          ...
          <CardSkeleton><div className="h-full w-full" /></CardSkeleton>
        </Card>
        ...
        <Card>
          ...
          <CardDescription>
            Preview and debug workflow logic in a safe sandbox deploying, helping you iterate with confidence.
          </CardDescription>
...
const CardSkeleton = ({
  ...
  return (
    <div
      className={cn("h-60 bg-gray-50 w-full my-4 rounded-lg overflow-hidden",
...
const CardTitle = ({
  ...
  return <h2 className={cn("font-medium text-lg trackng-tight", className)}>{children}</h2>;
};
